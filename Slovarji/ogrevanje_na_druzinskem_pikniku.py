...
def zdruzi_sladkosnede(domaci, njihovi):
    s = {}
    for x, y in njihovi.items():
        s[x] = y

    for x, y in domaci.items():
        s[x] = y
    return s
...
#     {'pita': 1}
# =============================================================================
def nakupovalni_seznam(s, t):
    seznam = {}
    for i in range(len(t)):
        if t[i] in s:
            if s[t[i]] not in seznam:
                seznam[s[t[i]]] = 1
            else:
                seznam[s[t[i]]] = int(seznam[s[t[i]]] + 1)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUwMH0:1iREpV:7yzs0GRrqpQ-aVRRB-gHjUGjGp8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUwMX0:1iREpV:kl66iXKJ648bl7Q3PSHh3DUQQTc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUwMn0:1iREpV:yFLpoG-0ne8a4BRyKW466L_rJmk'
        try:
