...
def cikel(permutacija, x):
    t = [x]
    start = x
    while slika(permutacija, x) != start:
        t.append(slika(permutacija, x))
        x = slika(permutacija, x)
...
def cikli(permutacija):
    t = []
    c = set()
    for i in permutacija:
        y = cikel(permutacija, i)
        if i not in c:
            t.append(y)
            c.update(y)
    return t
...
#     False
# =============================================================================
def je_permutacija(s):
    print(set(range(1, len(s)+1)))
    print(set(s.keys()))
    return set(s.keys()) == set(s.values()) == set(range(1, len(s)+1))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUxMH0:1iRGps:9mwKed4QlXhh5p1vn_R_PgQlnDA'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUxMX0:1iRGps:WAKQ3lDoy-LwTmHpvaUnf6l6oKs'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUxMn0:1iRGps:OollglbJ2QQ1vanFrfsNiAX5V7c'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUxM30:1iRGps:AYKi_Y8cTfik1NhN61YEAFB7KnU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTUxNH0:1iRGps:JSpcT-WoQ46q6Ozab5G_DUaOt5k'
        try:
