...
def v_uniji(x0, y0, krogi):
    for krog in krogi:
        dist = sqrt((x0-krog[0])**2+(y0-krog[1])**2)
        if dist <= krog[2]:
            return True
...
def pravokotnik(krogi):
    ...
    y_max = []
    for krog in krogi:
        x_min.append(krog[0] - krog[2])
        y_min.append(krog[1] - krog[2])
        x_max.append(krog[0] + krog[2])
        y_max.append(krog[1] + krog[2])
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ0OSwidXNlciI6NDUwMH0:1iTq0c:jKCBF9L1DhtTd5RyDC24dR84rMc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ1MCwidXNlciI6NDUwMH0:1iTq0c:jsEcjUFXfqh-5btRLSxMdjqyAO4'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ1MSwidXNlciI6NDUwMH0:1iTq0c:ppAnY5DtD1lxzW_LmlsZG19h9Ss'
        try:
