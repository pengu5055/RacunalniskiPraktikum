...
#    return True
def vsi_vecji(sez1, sez2):
    if len(sez1) and len(sez2) == 0:
        return True
    elif min(sez1) >= max(sez2):
        return True
    else:
        return False
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzOCwidXNlciI6NDUwMH0:1iSnqI:_Xx7WOKJbF-h0jQotJigzAc86P0'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzOSwidXNlciI6NDUwMH0:1iSnqI:dgRxHyrDyq6zKqq7ipO3ybGlNsU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ0MCwidXNlciI6NDUwMH0:1iSnqI:RqpfQrHEln-7dcnsVShH1ybgiRM'
        try:
            Check.equal('vsi_vecji()', False)
