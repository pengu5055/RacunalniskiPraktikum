...
def razpolovi_seznam(t): # Ne pozabi obstoj [:n] in [n:]. Je precej boljsa resitev
    length = len(t)
    l = t.copy()
...
#     5
# =============================================================================
def najvecji_element(t):  # Or built-in function max
    m = 0
    if not t:
        return None
    for i in t:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzMywidXNlciI6NDUwMH0:1iSnR5:iLUUUjqvX7_RZjLNvLjwG9CdK7s'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzNCwidXNlciI6NDUwMH0:1iSnR5:xlcDNPXHSQzsulyCEVWgIb9GSi0'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzNSwidXNlciI6NDUwMH0:1iSnR5:SkqL2Kt4MXA5KHab-IKrAdzfvMU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzNiwidXNlciI6NDUwMH0:1iSnR5:UcC5LyLnAAM2KhBTH0lPEQGVdng'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQzNywidXNlciI6NDUwMH0:1iSnR5:WzWQG9R6mJa5frSHqP4uFMhSIPc'
        try:
