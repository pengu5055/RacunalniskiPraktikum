...
def zivi(svet, i, j):
    c = 0 # Counter
    if svet[i][j]: #Check middle cell
        c -= 1
    for g in range(j-1, j+2):  # Iterate over x
        for h in range(i-1, i+2):  # Iterate over y
            try:
                if svet[h][g] and g > 0 and h > 0:
                    c += 1
            except IndexError:
                continue
...
import copy
def igra(svet):
    #novsvet = copy.deepcopy(svet)
    #novsvet = [x[:] for x in svet]#  Non lib solution [x[:] for x in svet]
    #for i in svet:
    #    for j in range(len(i)):
    #        state = (zivi(svet, svet.index(i), j), svet[svet.index(i)][j])  # Returns a tuple with live neighbor amount and cell state
#
    #        if state[0] < 2 and state[1]:  # Live cell dies with less than 2 live neighbors
    #            novsvet[svet.index(i)][j] = False
#
    #        elif 2 <= state[0] <= 3 and state[1]: # Live cell with 2 or 3 live neighbors lives
    #            novsvet[svet.index(i)][j] = True
#
    #        elif state[0] > 3 and state[1]:  # Live cell with more than 3 live neighbors dies
    #            novsvet[svet.index(i)][j] = False
#
    #        elif state[0] == 3 and not state[1]:  # Dead cell with 3 live neighbors gets resurrected
    #            novsvet[svet.index(i)][j] = True
#
    #return novsvet
    #return [[False if svet[svet.index(i)][j] and (zivi(svet, svet.index(i), j) < 2 or zivi(svet, svet.index(i), j) > 3) else True if
    #2 <= zivi(svet, svet.index(i), j) <= 3 or (not svet[svet.index(i)][j] and zivi(svet, svet.index(i), j) == 3) else False for j in range(len(i))] for i in svet]
    #return [[  for j in range(len(row))] for row in svet]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTkxOCwidXNlciI6NDUwMH0:1iwrXO:5bYqIR-n0K3WWuwZYeuQ5ikXUfY'
        try:
            svet_1 = [
                [False, False, False, False, False, False],
                [False, False, False, True, False, False],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTkxOSwidXNlciI6NDUwMH0:1iwrXO:SvM7nb2QVKaMjonhCd3UhR15NGs'
        try:
            svet_1 = [
                [False, False, False, False, False, False],
                [False, False, False, True, False, False],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTkyMCwidXNlciI6NDUwMH0:1iwrXO:sYi320VFxkjPer3ZjWaUTHLePtg'
        try:
            svet_1 = [
                [False, False, False, False, False, False],
                [False, False, False, True, False, False],
