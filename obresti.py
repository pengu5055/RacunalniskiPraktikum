...
def pretvori_v_letno_obrestno_mero(mesecna_obrestna_mera):
    return round((1 + mesecna_obrestna_mera)**12 * 100 - 100, 0)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQxOX0:1iLuQC:sPKTUXJePN16btIt-_EOY439XOw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQyMH0:1iLuQC:SZbuC4PJ8IDi0B9RyG39uWXTL94'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQyMX0:1iLuQC:Y6pbK898vEZO4-JLEKb0YdWTVhE'
        try:
            Check.equal('pretvori_v_letno_obrestno_mero(0)', 0)
            Check.equal('pretvori_v_letno_obrestno_mero(0.03)', 43)
            Check.equal('pretvori_v_letno_obrestno_mero(0.005)', 6)
            
            for mesecna in range(50):
                Check.secret(pretvori_v_letno_obrestno_mero(mesecna), mesecna)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
