...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQ3NX0:1iLwLX:cmRi_urEZFR8QbHqEOIN5UJx5eE'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQ3Nn0:1iLwLX:IqZOYRGd80HSu3OtiYndSwkr8dc'
        try:
            Check.equal("priblizek(0)", 0)
            Check.equal("priblizek(1)", 0)
            Check.equal("priblizek(2)", 0)
            Check.equal("priblizek(3)", 0)
            Check.equal("priblizek(4)", 0)
            Check.equal("priblizek(5)", 0)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQ3N30:1iLwLX:ULTbKsfQi1geaA63IBGBb9S1m2g'
        try:
