...
#     False
# =============================================================================
def je_seznam_ciklov(seznami):
    usednumbers = []
    for seznam in seznami:
        for element in seznam:
            if not element > 0 or seznam.count(element) != 1 or element in usednumbers:
                return False
            usednumbers.append(element)
...
def urejeni_cikli(seznami):
    t = []
    c = [] # Cycled list
    for seznam in seznami:
        if len(seznam) > 1:
            location = seznam.index(min(seznam)) -1
            #for i in seznam:
            #    c.insert((seznam.index(i)-location), i)
            #t.append(c.copy())
            #c.clear()
            t.append(seznam[-location:]+ seznam[:-location])
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5MSwidXNlciI6NDUwMH0:1iUceD:izH4DY4XFKtp2j2AD1XFUavBWqc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5MiwidXNlciI6NDUwMH0:1iUceD:RdkWlJioTU3r8FbZA-DlT_y29bw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5MywidXNlciI6NDUwMH0:1iUceD:yq5BqE8653UrUXZyrLe18cbyz90'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5NCwidXNlciI6NDUwMH0:1iUceD:oAAvTmi4HHXo0gEmXYcuLIj4NNA'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5NSwidXNlciI6NDUwMH0:1iUceD:sKL0lS6X1iwtudi8xzsW0-5I3ck'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5NiwidXNlciI6NDUwMH0:1iUceD:JusG3K-gwc5G9ZW-yIRaDSAa3rc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5NywidXNlciI6NDUwMH0:1iUceD:e9eYHVa4aYhsacPICPK2AL4iArM'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5OCwidXNlciI6NDUwMH0:1iUceD:NSFJXji9Y3oZr1Qnw6jK_RG_ngg'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5OSwidXNlciI6NDUwMH0:1iUceD:xc4BpFsc-2EJ_476Z7tRWDegr4s'
        try:
