...
def razbij_na_stevke(stevilo):
    return [int(numb) for numb in str(stevilo)]
# =====================================================================@021488=
# 2. podnaloga
...
def prestej_crke(geslo, niz):
    return [niz.count(i) for i in geslo.casefold()]
# =====================================================================@021489=
# 3. podnaloga
...
def sestej_stevke(stevke):
    ...
    for i in range(1, len(stevke)):
        if stevke[i-1]+stevke[i] > 9:
            for numb in razbij_na_stevke(stevke[i-1]+stevke[i]):
                t.append(numb)
        else:
            t.append(stevke[i-1]+stevke[i])
...
def ujemanje(oseba1, oseba2, geslo="LOVES"):
    t = [x+y for x,y in zip(prestej_crke(geslo, oseba1), prestej_crke(geslo, oseba2))]
    while len(t) > 2:
        t = sestej_stevke(t)

    return int(str(t[0]) + str(t[1]))

#def ujemanje(oseba1, oseba2, geslo='LOVES'): # Tudi solid resitev
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ4NywidXNlciI6NDUwMH0:1iUc1j:0sQIpOTyRWBx4M-0mEQM2_8Og_8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ4OCwidXNlciI6NDUwMH0:1iUc1j:UATtqXWwgDFxAVL3ucDgVCBxbik'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ4OSwidXNlciI6NDUwMH0:1iUc1j:DxkZC1eC8SJY1urIvhctiuts7YY'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMTQ5MCwidXNlciI6NDUwMH0:1iUc1j:jxO01H3E5qnZMc6izoTRKrUm1w4'
        try:
