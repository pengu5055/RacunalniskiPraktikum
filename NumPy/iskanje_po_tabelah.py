...
import numpy as np
def vecji(a, b):
    return np.indices(a.shape)[0][a > b]
# =====================================================================@022452=
# 2. podnaloga
...
# =============================================================================
def zdruzena(a, b):
    return np.where(a > 0, a, b) # Tm kjer je a > 0, iz vrednosti a, ce ne nadomesti z b
# =====================================================================@022453=
# 3. podnaloga
...
def lokalni_maksimum(a):
    print(a)
    a = np.pad(a, 1, "constant", constant_values=np.nan)
    #return np.indices(a.shape)[0][(np.roll(a, 1) > a) & (a < np.roll(a, -1)), a]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQ1MSwidXNlciI6NDUwMH0:1iwYhi:Rhbi4tlvmeeTLfZHQlj-J1dudeg'
        try:
            import numpy as np
            Check.approx('vecji(np.array([1, 1, 1]), np.array([0, 0, 0]))', np.array([0, 1, 2]))
            Check.approx('vecji(np.array([1, 1, 1, 1, 1]), np.array([2, 0, 2, 0, 2]))', np.array([1, 3]))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQ1MiwidXNlciI6NDUwMH0:1iwYhi:AF89CFDSuhrJ9cnNmGbhRKfO19c'
        try:
            import numpy as np
            Check.approx('zdruzena(np.array([1, -1, 1]), np.array([0, 0, 0]))', np.array([1, 0, 1]))
            Check.approx('zdruzena(np.array([2, 1, 0, -1, -2]), np.array([-2, -1, 0, 1, 2]))', np.array([2, 1, 0, 1, 2]))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQ1MywidXNlciI6NDUwMH0:1iwYhi:Uc3rUlt_Ygx6S5WnstjV5V2r2Yo'
        try:
            import numpy as np
            Check.approx('lokalni_maksimum(np.array([0, 1, 0]))', np.array([1]))
            Check.approx('lokalni_maksimum(np.array([2, 1, 3, 2, -1, 1]))', np.array([0, 2, 5]))
            Check.approx('lokalni_maksimum(np.array([-2, 1, 3, 2, -1, 1]))', np.array([2, 5]))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
