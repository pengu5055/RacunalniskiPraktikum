...
# podano natančnost.
# =============================================================================
def natancni_priblizek(a):
    k = 0
    dif = naslednji_priblizek(priblizek(k)) - priblizek(k)
    while priblizek(k) - naslednji_priblizek(priblizek(k)) > a:
        print("loop")
        k += 1
    return 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQ3NX0:1iM7oD:_IaxCrYaLMmyVs70JysRpgJW5kU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQ3Nn0:1iM7oD:7wc3FU9dHQdr8i00AHp5aqunM5U'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo0NTAwLCJwYXJ0IjoyMTQ3N30:1iM7oD:BDLuqaEBbtx3gegKk8ghiQIHPHQ'
        try:
            Check.equal("natancni_priblizek(0.1)", 0)
            Check.equal("natancni_priblizek(0.05)",0)
            Check.equal("natancni_priblizek(0.01)",0)
            Check.equal("natancni_priblizek(0.001)", 0)
            Check.equal("natancni_priblizek(0.005)", 0)
            Check.equal("natancni_priblizek(0.0001)", 0)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
